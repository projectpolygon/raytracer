...
public:
    ...
    void multithread_render_scene(World& world, unsigned int num_threads)
    {
      ...
      std::vector<std::shared_ptr<scene_slab>> slabs;
      ...
      for (int i = 0; i < wheight; i += world.m_slab_size)
      {
        ...
        for (int j = 0; j < wwidth; j += world.m_slab_size)
        {
          ...
          std::shared_ptr<scene_slab> new_ti = std::make_shared<scene_slab>(
      ...
      for (unsigned int i = 0; i < num_threads; i++) {
        thread_list.push_back(std::thread(
          ...
          {
              std::shared_ptr<scene_slab> slab_ptr = nullptr;
...
protected:
    void render_slab(std::shared_ptr<scene_slab> slab) const
    {
