...
namespace poly::object
{
								   ...
								   float t) const
	{
		...
		float ab_squared = m_a * m_a + m_b * m_b;
		float n_x = 4 * point.x * (xyz_squared - ab_squared);
		float n_y = 4 * point.y * (xyz_squared - ab_squared + (2 * m_a * m_a));
	...
	std::vector<double> Torus::rcoeffs_get(math::Ray<math::Vector> const &R) const
	{
		...
		double e = glm::dot(O, O) - (m_a * (double)m_a) - (m_b * (double)m_b);
		...
		double c_2 = 2.0 * sum_d_sqrd * e + 4.0 * od * od + four_a_sqrd * (double)D.y * D.y;

		double c_1 = 4.0 * od * e + 2.0 * four_a_sqrd * O.y * D.y;

		double c_0 = e * e - four_a_sqrd * ((double)m_b * m_b - (double)O.y * O.y);
