...
namespace poly::structures
{
	...
	/**
	Creates a vector of slabs ranging that renders point from [start_x, end_x)
	and [start_y, end_y)
	...
	@returns std::vector<std::shared_ptr<poly::structures::scene_slab>> list of
	scene slab pointers
	*/
	std::vector<std::shared_ptr<poly::structures::scene_slab>>
	generate_slabs(int start_x,
				   int end_x,
				   int start_y,
				   int end_y,
				   ...
				   std::shared_ptr<std::vector<std::vector<Colour>>> storage)
	{
		...
		for (int i = corrected_start_y; i < corrected_end_y;
			 i += static_cast<int>(preferred_slab_size)) {
			...
			for (int j = corrected_start_x; j < corrected_end_x;
				 j += static_cast<int>(preferred_slab_size)) {
