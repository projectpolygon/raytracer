...
namespace poly::material
{
	...
	{}
	PerfectTransmitter::PerfectTransmitter(float kt, float _ior) :
		BTDF(kt, _ior)
	{}

	bool PerfectTransmitter::tot_int_refl(
		poly::structures::SurfaceInteraction const &sr) const
	{
	...
	Colour
	PerfectTransmitter::sample_f(poly::structures::SurfaceInteraction const &sr,
								 ...
								 atlas::math::Vector &w_t) const
	{
		...
		Colour retcol = (m_kt / (eta * eta)) *
						(Colour(1.0f, 1.0f, 1.0f) *
						 (1.0f / (std::abs(glm::dot(sr.m_normal, w_t)))));
		return retcol;
	}

	Colour PerfectTransmitter::rho(
		[[maybe_unused]] poly::structures::SurfaceInteraction const &sr,
